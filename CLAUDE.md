...
### 12.1 Povinný AI/design workflow (závazné)

Primárním vývojovým nástrojem projektu je **Claude Code**. Povinný design/dev
řetězec pro veškerou práci na UI:

1. **Claude Code** — primární vývojové workflow (implementace, refactor, QA, drobné commity).
2. **Motion (Framer Motion)** — Framer Motion používáme přes aktuální balík `motion`
   a importy `motion/react`. Starý balík `framer-motion` nepřidávat. Každá animace
   musí respektovat `useReducedMotion`.
3. **`impeccable`** — povinná UI/UX quality vrstva. For all visual/UI work, use the
   impeccable skill as the mandatory UI/UX quality layer. (Skill „UI/UX Pro Max“ pod
   tímto názvem neexistuje — ekvivalent je `impeccable`.)
4. **21st.dev Magic MCP** — generování / inspirace moderních UI komponent (volitelně
   aktivní, povinně podporovaný; vyžaduje user-env API key — viz `docs/mcp-ui-workflow.local.md`).
5. **Chrome DevTools MCP** — browser debug: konzole, síťové requesty, performance
   profiling, runtime chyby, podklad pro Lighthouse/performance audit.
6. **Playwright screenshot loop** — vizuální QA na 1440 / 768 / 390.

**Pravidlo:** Claude nesmí dělat UI změny pouze podle textového dojmu. U významných
UI změn musí použít minimálně screenshot workflow, a pokud jde o
runtime/performance/browser problém, také Chrome DevTools MCP.
